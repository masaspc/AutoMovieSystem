...
このファイルは Claude Code / Codex 双方が読む共通の正本(durable repo conventions)です。
Claude Code は `CLAUDE.md` から本ファイルを import し、Codex は `AGENTS.md` を直接読み込みます。
...
## スキル(Claude Code / Codex 共有)

プロジェクト固有スキルの実体は **`.agents/skills/`**(git管理下の単一の正本)に置く。
各ツールのスキル探索先へはOS別のリンクで接続する(内容の二重管理をしない):

- `.claude/skills` → `.agents/skills`(Claude Code のプロジェクトスキル)
- `.codex/skills` → `.agents/skills`(Codex のプロジェクトスキル)
- `~/.codex/skills/<name>` → `.agents/skills/<name>`(Codex はここ=`$CODEX_HOME/skills` を自動発見する)

これらのリンクは環境依存のため git 管理外(`.gitignore` 済み)。**チェックアウト直後に一度だけ**
セットアップスクリプトを実行してリンクを張る:

```
# Windows(管理者権限不要のディレクトリjunctionを使用)
...
スキルの追加・編集は `.agents/skills/<name>/SKILL.md` に対して行えば、リンク経由で両ツールへ反映される。
現在のスキル: `diagnose-failure` / `implement-feature` / `review-change` / `verify-mvp`。
