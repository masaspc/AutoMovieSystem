...
## D-029: チャンネル編集方針はJSONで保持し、金融安全策は共通ゲートで強制する(2026-07-18)

チャンネル固有のトーン、対象視聴者、禁止事項、免責文、トレンドフィード、既定制作設定は
`channels.editorial_policy` のSQLite互換JSONとして保持する。不正な既存値は警告して空ポリシーへ
戻すfail-softとし、チャンネル別フィードが空ならグローバル設定へフォールバックする。

一方、「必ず儲かる」「絶対に上がる」「損しない」等の断定表現はチャンネル設定に依存しない
共通blockingルールとする。免責文が設定されたチャンネルでは概要欄へ重複なく自動挿入し、
アップロード直前にも挿入済みdescriptionを検証して欠落時はfail-closedとする。免責文は
エンディングCTAにも表示し、その内容をレンダー入力チェックサムへ含める。
